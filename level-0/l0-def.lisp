...
(defun %defun (named-fn &optional info)
  ...
  (when (not (and (fboundp '%defun-encapsulated-maybe)
                  (%defun-encapsulated-maybe name named-fn)))
    (when (and (symbolp name) (fboundp 'macro-function) (macro-function name nil))
      (warn "The macro ~s is being redefined as a function." name)
      (fmakunbound name))
    (if (fboundp 'fset)
      (fset name named-fn)
      (%fhave name named-fn)))
  (when (fboundp 'set-function-info)
    (set-function-info name info))
  (when (and *fasload-print* (fboundp 'format))
    (format t "~&~S~%" name))
  name))
