...
;;; Bug 165e: MVB-free variants — return (cons str (cons len new-p)) instead of values.
(defun %fasl-vreadstr-triple (s)
  (let* ((nchars (%fasl-read-count s))
         (nextra (%fasl-read-count s))
         (copy t)
         (n nchars)
    ...
    (if (> n (length str))
      ...
      (setq copy nil))
    (%fasl-read-utf-8-string s str nchars nextra)
    (cons str (cons nchars copy))))

(defun %fasl-nvreadstr-triple (s)
  (let* ((nchars (%fasl-read-count s))
         (copy t)
         (n nchars)
         (str (faslstate.faslstr s)))
...
(defun %fasl-vintern (s package &optional binding-index)
  (let* ((str-triple (%fasl-vreadstr-triple s))
         (str (car str-triple))
         (len (cadr str-triple))
         (new-p (cddr str-triple)))
    (with-package-lock (package)
      (let* ((find-result (%find-symbol-quad str len package))
             (symbol (car find-result))
             (access (cadr find-result))
             (internal-offset (caddr find-result))
             (external-offset (cdddr find-result)))
        (unless access
...
(defun %fasl-nvintern (s package &optional binding-index)
  (let* ((str-triple (%fasl-nvreadstr-triple s))
         (str (car str-triple))
         (len (cadr str-triple))
         (new-p (cddr str-triple)))
    (with-package-lock (package)
...
;;; Bug 165e: Avoid MVB — use cons-returning variants of %fasl-vreadstr/nvreadstr.
(defun %fasl-vpackage (s)
  (let* ((triple (%fasl-vreadstr-triple s))
         (str (car triple))
         (len (cadr triple))
         (new-p (cddr triple))
         (p (%find-pkg str len)))
...
(defun %fasl-nvpackage (s)
  (let* ((triple (%fasl-nvreadstr-triple s))
         (str (car triple))
         (len (cadr triple))
         (new-p (cddr triple))
         (p (%find-pkg str len)))
...
;;; Bug 165e: Avoid MVB — use cons pair from hash-pname.
(defun %hash-pname-pair (str len)
  (declare (optimize (speed 3) (safety 0)))
  (let* ((primary (mixup-hash-code (%pname-hash str len))))
    (declare (fixnum primary))
    (cons primary (aref (the (simple-array (unsigned-byte 16) (8)) $hprimes) (logand primary 7)))))

(defun %get-htab-symbol (string len htab)
  (declare (optimize (speed 3) (safety 0)))
  ;; Bug 165e: Avoid MVB — use cons pair.
  (let* ((pair (%hash-pname-pair string len))
         (p (car pair))
         (s (cdr pair)))
    (%get-hashed-htab-symbol string len htab p s)))

;;; Bug 165e: %get-htab-symbol returns (values found-p sym offset).
;;; Cons-returning variant to avoid MVB in callers.
(defun %get-htab-symbol-triple (string len htab)
  (declare (optimize (speed 3) (safety 0)))
  (let* ((pair (%hash-pname-pair string len))
         (p (car pair))
         (s (cdr pair)))
    ...
    (let* ((vec (htvec htab))
           (vlen (length vec)))
      (declare (fixnum vlen))
      (do* ((idx (fast-mod p vlen) (+ i s))
            (i idx (if (>= idx vlen) (- idx vlen) idx))
            (elt (%svref vec i) (%svref vec i)))
           ((eql 0 elt) (cons nil (cons nil i)))
        (declare (fixnum i idx))
        (when (symbolp elt)
          (let* ((pname (symbol-name elt)))
            (if (and
                 (= (the fixnum (length pname)) len)
                 (dotimes (j len t)
                   (unless (eq (schar string j) (schar pname j))
                     (return))))
              (return (cons t (cons (%symptr->symbol elt) i))))))))))

(defun %find-symbol (string len package)
  (declare (optimize (speed 3) (safety 0)))
  ;; Bug 165e: Avoid all MVBs — use cons-returning %get-htab-symbol-triple.
  (let* ((itab-result (%get-htab-symbol-triple string len (pkg.itab package)))
         (found-p (car itab-result))
         (sym (cadr itab-result))
         (internal-offset (cddr itab-result)))
    (if found-p
      (values sym :internal internal-offset nil)
      (let* ((etab-result (%get-htab-symbol-triple string len (pkg.etab package)))
             (found-p2 (car etab-result))
             (sym2 (cadr etab-result))
             (external-offset (cddr etab-result)))
        (if found-p2
          (values sym2 :external internal-offset external-offset)
          (dolist (p (pkg.used package) (values nil nil internal-offset external-offset))
            (let* ((used-result (%get-htab-symbol-triple string len (pkg.etab p)))
                   (found-p3 (car used-result))
                   (sym3 (cadr used-result)))
              (when found-p3
                (return (values sym3 :inherited internal-offset external-offset))))))))))
          
;;; Bug 165e: Cons-returning %find-symbol for %fasl-vintern/nvintern.
;;; Returns (cons symbol (cons access (cons internal-offset external-offset))).
(defun %find-symbol-quad (string len package)
  (declare (optimize (speed 3) (safety 0)))
  (let* ((itab-result (%get-htab-symbol-triple string len (pkg.itab package)))
         (found-p (car itab-result))
         (sym (cadr itab-result))
         (internal-offset (cddr itab-result)))
    (if found-p
      (cons sym (cons :internal (cons internal-offset nil)))
      (let* ((etab-result (%get-htab-symbol-triple string len (pkg.etab package)))
             (found-p2 (car etab-result))
             (sym2 (cadr etab-result))
             (external-offset (cddr etab-result)))
        (if found-p2
          (cons sym2 (cons :external (cons internal-offset external-offset)))
          (dolist (p (pkg.used package) (cons nil (cons nil (cons internal-offset external-offset))))
            (let* ((used-result (%get-htab-symbol-triple string len (pkg.etab p)))
                   (found-p3 (car used-result))
                   (sym3 (cadr used-result)))
              (when found-p3
                (return (cons sym3 (cons :inherited (cons internal-offset external-offset))))))))))))
