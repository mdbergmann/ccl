...
(defmacro lfunloop (for var in function &body loop-body)
  ...
  (let ((fn (gensym))
    ...
    `(loop with ,fn = ,function
           ...
           for ,i from #+ppc-target 1 #+x86-target (%function-code-words ,fn) #+(or arm-target arm64-target) 2  below (%i- (uvsize  ,lfv) 1)
           as ,var = (%svref ,lfv ,i)
           ,@loop-body)))
...
(defmacro with-utf-8-cstr ((sym str) &body body)
  ;; Bug 165e: Avoid MVB (ARM64 vstack corruption) — use cons-returning variant.
  (let* ((pair (gensym))
         (data (gensym))
         (offset (gensym))
         (string (gensym))
    ...
    `(let* ((,string ,str)
            (,len (length ,string))
            (,pair (%array-data-and-offset-cons ,string))
            (,data (car ,pair))
            (,offset (cdr ,pair))
            (,end (+ ,offset ,len))
