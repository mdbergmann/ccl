...
;;; ARM64 backtrace support.
;;; Port of arm-backtrace.lisp for the ARM64 architecture.

(in-package "CCL")

(defun cfp-lfun (p)
  (if (fake-stack-frame-p p)
    (let* ((fn (%fixnum-ref p arm64::fake-stack-frame.fn))
           (lr (%fixnum-ref p arm64::fake-stack-frame.lr)))
...
;;; ARM64 has no saved registers in the lisp frame (only savevsp + savelr).
...
;;; ARM64 is 64-bit; fixnumshift=0 so address = value directly.
(defun index->address (p)
  (when (fake-stack-frame-p p)
    (setq p (%fixnum-ref p arm64::fake-stack-frame.sp)))
  (ldb (byte 64 0) (ash p arm64::fixnumshift)))
...
(defun %frame-savevsp (p)
  (if (fake-stack-frame-p p)
    (%fixnum-ref p arm64::fake-stack-frame.vsp)
    (%%frame-savevsp p)))

;;; Lexprs
(defun arg-check-call-arguments (frame function)
  (declare (ignore function))
  (xp-argument-list (%fixnum-ref frame arm64::fake-stack-frame.xp)))
...
;;; ARM64 word-shift is 3 (8-byte words).
