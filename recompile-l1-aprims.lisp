;;; Recompile just l1-aprims.lisp for ARM64 via the cross-compile mechanism
;;; Usage: ./dx86cl64 -l recompile-l1-aprims.lisp
(in-package :ccl)
...
;;; Step 4: Compile l1-aprims.lisp targeting ARM64 using cross-compile mechanism
(format t "~%=== Compiling l1-aprims.lisp for ARM64 ===~%")
(with-cross-compilation-target (:darwinarm64)
  (let* ((*target-backend* (find-backend :darwinarm64)))
    (target-compile-modules '(l1-aprims) :darwinarm64 t)))
(format t "~%=== Done ===~%")
(quit)
