...
(defun arm642-one-untargeted-lreg-form (seg form reg)
  (arm642-one-lreg-form seg form (if (typep reg 'lreg) reg (make-unwired-lreg reg))))
  ...
  (or (arm642-reg-for-form form suggested)
      (if (and (acode-p form)
        ...
        (if (node-reg-p suggested)
          (arm642-one-untargeted-lreg-form seg form (arm642-try-non-conflicting-reg suggested reserved))
...
(defun arm642-vpush-register (seg src)
  (with-arm64-local-vinsn-macros (seg)
    (prog1
      (! vpush-register src)
      ...
      (let* ((dest-gpr (backend-ea-physical-reg dest hard-reg-class-gpr))
        ...
        (if (null src)
          ...
          (if dest-crf
            ...
            ;; On ARM64, use rnil register (nil value is too large for CMP immediate).
            (if src-gpr
                ...
                (#.hard-reg-class-gpr-mode-node ; boxed result.
                 (case src-mode
                   ...
                   (#.hard-reg-class-gpr-mode-u32
                    (arm642-box-u32 seg dest src))
                   (#.hard-reg-class-gpr-mode-s32
                    (arm642-box-s32 seg dest src))
                      ...
                      (#.hard-reg-class-gpr-mode-node
                       (case dest-mode
                         ...
                         (#.hard-reg-class-fpr-mode-complex-single-float
                          (unless *arm642-reckless*
                            (! trap-unless-typecode= src arm64::subtag-complex-single-float))
                          (! get-complex-single-float dest src))
                         (#.hard-reg-class-fpr-mode-complex-double-float
                          (unless *arm642-reckless*
                            (! trap-unless-typecode= src arm64::subtag-complex-double-float))
...
;;; ======================================================================
;;; Section 7: Symbol/Function Support (Chunk 6)
;;; ======================================================================
...
(defun arm642-symbol-entry-locative (sym)
  ...
  (when (eq sym '%call-next-method-with-args)
    (setf (afunc-bits *arm642-cur-afunc*)
          (%ilogior (%ilsl $fbitnextmethargsp 1) (afunc-bits *arm642-cur-afunc*))))
...
(defun arm642-afunc-lfun-ref (afunc)
  (or
   (afunc-lfun afunc)
   (progn (pushnew afunc (afunc-fwd-refs *arm642-cur-afunc*) :test #'eq)
          afunc)))
    ...
    (let* ((nregs (length revregargs))
      ...
      (when revregargs
        (let* ((zform (%car revregargs))
          ...
          (if (eq 3 nregs)
            (arm642-three-targeted-reg-forms seg xform ($ arm64::arg_x) yform ($ arm64::arg_y) zform ($ arm64::arg_z))
            (if (eq 2 nregs)
              (arm642-two-targeted-reg-forms seg yform ($ arm64::arg_y) zform ($ arm64::arg_z))
...
;;; ======================================================================
;;; Section 8: Function Calling Infrastructure (Chunk 6)
;;; ======================================================================
...
;;; On ARM64: no dedicated fn register, use nfn everywhere.
;;; ARM64 has no NVRs ($numarm64saveregs=0) but keep restore calls as no-ops.
...
;;; ======================================================================
;;; Section 9: MV/Control Flow Support (Chunk 6)
;;; ======================================================================
...
;;; ======================================================================
;;; Section 10: Closure & Builtin Support (Chunk 6)
;;; ======================================================================
...
(defun arm642-seq-fbind (seg vreg xfer vars afuncs body p2decls)
  (let* ((old-stack (arm642-encode-stack))
         (copy afuncs)
         (func nil))
    (with-arm642-p2-declarations p2decls
      (dolist (var vars)
        ...
        (if downward-p
          (progn
            (let* ((*arm642-vstack* *arm642-vstack*))
              ...
              (arm642-vpush-register-arg seg arm64::temp0)
          ...
          (let* ((cell 1))
            ...
            (progn
              ...
              (! %alloc-misc-fixed dest arm64::imm0 (ash vsize (arch::target-word-shift arch))))
            (! %closure-code% arm64::arg_x)
            (! %codevector-entry arm64::lr arm64::arg_x)
            (! misc-set-c-node arm64::lr dest 0)
...
(defun arm642-ternary-builtin (seg vreg xfer name form1 form2 form3)
  (with-arm64-local-vinsn-macros (seg)
    (arm642-three-targeted-reg-forms seg form1 ($ arm64::arg_x) form2 ($ arm64::arg_y) form3 ($ arm64::arg_z))
    (arm642-fixed-call-builtin seg vreg xfer name)))

;;; ======================================================================
;;; Section 11: defarm642 Macro + Call Operators (Chunk 6)
;;; ======================================================================
      ...
      (destructuring-bind (vcode-block dest control &rest other-args) arglist
        (let* ((fun `(nfunction ,name
                      (lambda (,vcode-block ,dest ,control ,@other-args) ,@decls
                              (block ,name (with-arm64-local-vinsn-macros (,vcode-block ,dest ,control) ,@body))))))
          `(progn
            (record-source-file ',name 'function)
            (svset *arm642-specials* (%ilogand #.operator-id-mask (%nx1-operator ,locative)) ,fun))))))
  )
...
;;; ======================================================================
;;; Section 12: Additional Helpers (Chunk 6)
;;; ======================================================================

;;; On ARM64, fixnumshift=0 so val is just the raw fixnum value.
;;; ARM64 add/sub immediates: unsigned 12-bit (< 4096).
;;; For CMP we can use either CMP (subtract) or CMN (add), so check abs(val).
...
;;; ======================================================================
;;; Section 8: Memory Store, Stack Encoding, Utilities & Control Flow (Chunk 7)
;;; ======================================================================

;;; --- Memory store: halfword boxing ---

(defun arm642-store-signed-halfword (seg vreg imm-reg)
  (with-arm64-local-vinsn-macros (seg vreg)
    (when (arm642-for-value-p vreg)
      (if (logbitp vreg arm64-imm-regs)
        (<- imm-reg)
        ;; On ARM64 with fixnumshift=0, sign-extended s16 is already a valid fixnum
        (! box-fixnum vreg imm-reg)))))

(defun arm642-store-unsigned-halfword (seg vreg imm-reg)
  (with-arm64-local-vinsn-macros (seg vreg)
    (when (arm642-for-value-p vreg)
      (if (logbitp vreg arm64-imm-regs)
        (<- imm-reg)
        ;; On ARM64 with fixnumshift=0, zero-extended u16 is already a valid fixnum
        (! box-fixnum vreg imm-reg)))))

;;; --- Memory store: pointer set ---
...
;;; --- Memory store: displaced and indexed ---
...
;;; --- Memory store: general immediate store ---
...
(defun arm642-open-undo (&optional (reason $undocatch) (curstack (arm642-encode-stack)))
  (set-fill-pointer
   *arm642-undo-stack*
...
;;; --- Initialized gvector allocation ---
...
;;; --- Cons modification (rplaca/rplacd) ---
        ...
        (if (eq xfer $backend-return)
          ...
          (if (not (arm642-cd-compound-p xfer))
            ...
            (let* ((truebranch (arm642-cd-true xfer))
              ...
              (unless cr-bit (setq cr-bit arm64::arm64-cond-eq))
...
;;; --- Compound destination (CD) utilities ---
...
(defun arm642-undo-body (seg vreg xfer body old-stack)
  (let* ((current-stack (arm642-encode-stack))
         (numundo (%i- *arm642-undo-count* (arm642-encoding-undo-count old-stack))))
        ...
        (if (eq xfer $backend-return)
          ...
          (if (arm642-mvpass-p xfer)
            (progn
              ...
              (dotimes (i numundo) (arm642-close-undo)))
            (progn
              (arm642-form seg (if (or vreg (not (%izerop numundo))) arm64::arg_z) nil body)
...
;;; --- Register variable init ---

(defun arm642-init-regvar (seg var reg addr)
  (with-arm64-local-vinsn-macros (seg)
    (arm642-stack-to-register seg addr reg)
    (arm642-set-var-ea seg var (make-wired-lreg reg :class (hard-regspec-class reg) :mode (get-regspec-mode reg)))))
          ...
          (flet ((popnlispareas ()
            ...
            (when (and (neq lastcatch dest)
                       (%i>
                        vstack
                        (setq target-vstack
                              (nth-value 2 (arm642-decode-stack (aref *arm642-undo-stack* dest)))))
                       (neq retval t))
              (unless returning
...
;;; ======================================================================
;;; Section 13: Missing Utility Functions (Chunk 9, Part A)
;;; ======================================================================

;;; --- Multi-form register allocation utilities ---

(defun arm642-push-reg-for-form (seg form suggested &optional targeted)
  (let* ((reg (if (and (node-reg-p suggested)
                       (nx2-acode-call-p form))
                (arm642-one-targeted-reg-form seg form arm64::arg_z)
                (if targeted
...
(defun arm642-copy-fpr (seg dest src)
  ...
  (with-arm64-local-vinsn-macros (seg)
    (case (fpr-mode-value-name (get-regspec-mode src))
      (:single-float (! copy-gpr dest src))
      (:double-float (! copy-gpr dest src))
      (:complex-single-float (! copy-gpr dest src))
      (:complex-double-float (! copy-gpr dest src)))))

;;; --- Two-form register allocation ---
...
;;; --- Three-form register allocation ---
...
;;; --- Unboxed integer / macptr arg to register ---
      ...
      (let* ((value (arm642-integer-constant-p form mode)))
        (if value
            (progn
              ...
              immreg)
          (progn
            (arm642-one-targeted-reg-form seg form (make-wired-lreg arm64::imm0 :mode modeval))))))))
...
;;; ARM64: CMP immediate accepts 0-4095 (unsigned 12-bit).
;;; For values outside range, load into temp register and compare.
...
;;; Main EQ/NEQ comparison dispatcher.
;;; ARM64: skip u8 optimization path (not yet implemented).
        ...
        (let* ((reg (arm642-one-untargeted-reg-form seg (if jconst i j) arm64::arg_z)))
          (if (typep (or jconst iconst) '(unsigned-byte 12))
            ...
            (with-node-target (reg) other
              ...
              (! compare vreg reg other)))
          (unless (or jconst (eq cr-bit arm64::arm64-cond-eq))
...
(defun arm642-branch-unless-arg-fixnum (seg reg label)
  (with-arm64-local-vinsn-macros (seg)
    (with-crf-target () crf
      (! test-fixnum crf reg)
      (! cbranch-false label crf arm64::arm64-cond-eq))))
...
;;; ARM64 char-p: extract tag byte, compare to tag-character (0x11).
;;; No mask-base-char needed — just extract-tag and compare.
  ...
  (with-arm64-local-vinsn-macros (seg vreg xfer)
    (multiple-value-bind (cr-bit true-p) (acode-condition-to-arm64-cr-bit cc)
      (let* ((reg (arm642-one-untargeted-reg-form seg form arm64::arg_z)))
        (with-imm-target () tag-reg
          (! extract-tag tag-reg reg)
          (arm642-test-reg-%izerop seg vreg xfer tag-reg cr-bit true-p
                                   arm64::tag-character))))))
        ...
        (if (or (eq typespec t)
          ...
          (let* ((ok (backend-get-next-label)))
            ...
            (with-crf-target () crf
               ...
               (! cbranch-false (aref *backend-labels* ok) crf arm64::arm64-cond-eq))
...
;;; --- Dynamic-extent form evaluation ---
                   ...
                   (if cval
                       ...
                       (with-crf-target () crf
                         (let ((stack-block-0-label (backend-get-next-label))
                           ...
                           (! cbranch-false (aref *backend-labels* stack-block-0-label) crf arm64::arm64-cond-eq)
                           (! make-stack-block)
                           (-> done-label)
                           (@ stack-block-0-label)
                           (! make-stack-block0)
...
;;; ======================================================================
;;; Section 14: Operator Definitions (Chunk 9, Part B)
;;; ======================================================================
...
(defarm642 arm642-setq-lexical setq-lexical (seg vreg xfer varspec form)
  (let* ((ea (var-ea varspec)))
    (if (and (memory-spec-p ea)
      ...
      (let* ((reg (arm642-one-untargeted-reg-form seg form (arm642-reg-for-nfp-set vreg ea))))
...
;;; ARM64: fixnumshift=0, so boxed = value.  No encode-arm-immediate check needed.
...
(defarm642 arm642-vector vector (seg vreg xfer arglist)
  (arm642-allocate-initialized-gvector seg vreg xfer
    ...
    (if (null vreg)
      ...
      (if (null subtag)
        (progn
          ...
          (<- arm64::arg_z)
          (^))
        (arm642-allocate-initialized-gvector seg vreg xfer subtag (cdr all-on-stack))))))
...
(defarm642 arm642-%slot-ref %slot-ref (seg vreg xfer instance idx)
  (ensuring-node-target (target (or vreg ($ arm64::arg_z)))
    (multiple-value-bind (v i)
        (arm642-two-untargeted-reg-forms seg instance arm64::arg_y idx arm64::arg_z)
      (unless *arm642-reckless*
...
(defarm642 arm642-svref svref (seg vreg xfer vector index)
  (arm642-vref seg vreg xfer :simple-vector vector index (unless *arm642-reckless* (nx-lookup-target-uvector-subtag :simple-vector))))

(defarm642 arm642-%sbchar %sbchar (seg vreg xfer string index)
  (arm642-vref seg vreg xfer :simple-string string index (unless *arm642-reckless* (nx-lookup-target-uvector-subtag :simple-string))))
...
(defarm642 arm642-svset svset (seg vreg xfer vector index value)
  (arm642-vset seg vreg xfer :simple-vector vector index value (nx-lookup-target-uvector-subtag :simple-vector)))
...
;;; ARM64: skip immediate optimization (bitmask encoding is complex).
;;; Always use register form.
...
(defarm642 arm642-%ilogxor2 %ilogxor2 (seg vreg xfer form1 form2)
  (let* ((fix1 (acode-fixnum-form-p form1))
         (fix2 (acode-fixnum-form-p form2)))
    (if (and fix1 fix2)
      (arm642-use-operator (%nx1-operator fixnum) seg vreg xfer (logxor fix1 fix2))
      (progn
        (multiple-value-bind (r1 r2) (arm642-two-untargeted-reg-forms seg form1 arm64::arg_y form2 arm64::arg_z)
...
;;; ARM64: max shift = 55 (56-bit fixnums), not 31 (ARM32).
...
(defarm642 arm642-char-code char-code (seg vreg xfer c)
  (arm642-extract-charcode seg vreg xfer c (not (arm642-form-typep c 'character))))
...
;;; ======================================================================
;;; Chunk 10: Comparisons, control flow, arithmetic, string/char,
;;;           float ops, natural ops, values
;;; ======================================================================
...
;;; ARM64: No ARM-style encode-arm-immediate; use ADD/SUB 12-bit imm check.
(defun arm642-constant-for-compare-p (form &optional unboxed)
  (setq form (acode-unwrapped-form form))
  (when (acode-p form)
    (let* ((op (acode-operator form)))
      (if (eql op (%nx1-operator fixnum))
        (let* ((val (if unboxed
                      (car (acode-operands form))
                      ;; fixnumshift=0 on ARM64, so no shifting needed
                      (car (acode-operands form)))))
          (if (and val (typep val '(signed-byte 12)))
...
;;; ARM64 doesn't have ARM32-style conditional execution/predication.
;;; This is a no-op stub.
(defun arm642-predicate-block (labelnum)
                  ...
                  (let* ((reg (arm642-one-untargeted-reg-form seg (make-acode (%nx1-operator lexical-reference) var) arm64::arg_z)))
                    (with-imm-target () (idx :u64)
                      (with-crf-target () flags
                        ...
                        (! cbranch-false (aref *backend-labels* defaultlabel) flags arm64::arm64-cond-lo)
      ...
      (if (and boolean (or iconst jconst))
        (let* ((reg (arm642-one-untargeted-reg-form seg (if jconst i j) ($ arm64::imm0 :mode :u64))))
          ...
          (unless (or jconst (eq cr-bit arm64::arm64-cond-eq))
            (setq cr-bit (arm642-cr-bit-for-reversed-comparison cr-bit)))
          (^ cr-bit true-p))
        (if (and (eq cr-bit arm64::arm64-cond-eq)
                 (or jconst iconst))
          (arm642-test-reg-%izerop
           seg vreg xfer
...
(defun arm642-inline-numcmp (seg vreg xfer cc name form1 form2)
  (with-arm64-local-vinsn-macros (seg vreg xfer)
    (multiple-value-bind (cr-bit true-p) (acode-condition-to-arm64-cr-bit cc)
      (let* ((otherform (and (eql cr-bit arm64::arm64-cond-eq)
...
;;; ARM64: fixnumshift=0, so "unboxed" fixval = fixval (no shifting).
;;; ARM64 ADD/SUB take 12-bit immediates, not ARM32 rotated-immediate.
    ...
    (let* ((fix1 (acode-fixnum-form-p form1))
      ...
      (if (and fix1 fix2)
        ...
        (let* ((fixval (or fix1 fix2))
               ...
               (done (backend-get-next-label)))
          ;; ARM64: no immediate form optimization (logical imm encoding is complex);
          ;; always use register form for inline path.
          (arm642-two-targeted-reg-forms seg form1 ($ arm64::arg_y) form2 ($ arm64::arg_z))
          (ensuring-node-target (target vreg)
            (if (acode-fixnum-form-p form1)
              (arm642-branch-unless-arg-fixnum seg ($ arm64::arg_z) (aref *backend-labels* out-of-line))
            ...
            (-> done)
            (@ out-of-line)
            (! call-subprim-2 ($ arm64::arg_z) (subprim-name->offset '.SPbuiltin-logior) ($ arm64::arg_y) ($ arm64::arg_z))
            (@ done)
            (arm642-copy-register seg target ($ arm64::arg_z)))
          (^))))))
      ...
      (if (and fix1 fix2)
        ...
        (let* ((out-of-line (backend-get-next-label))
          ...
          (ensuring-node-target (target vreg)
            ...
            (-> done)
            (@ out-of-line)
            (! call-subprim-2 ($ arm64::arg_z) (subprim-name->offset '.SPbuiltin-logand) ($ arm64::arg_y) ($ arm64::arg_z))
            (@ done)
            (arm642-copy-register seg target ($ arm64::arg_z)))
          (^))))))

;;; ARM64 fixnum-add: fixnumshift=0, so no shifting of constants.
;;; ARM64 ADD/SUB immediate is 12-bit (0..4095), not ARM32 rotated-immediate.
(defun arm642-fixnum-add (seg vreg xfer form1 form2 overflow)
  (with-arm64-local-vinsn-macros (seg vreg xfer)
    ...
    (let* ((fix1 (acode-fixnum-form-p form1))
           ...
           (other (unless sum
                    (if (and fix1 (typep fix1 '(unsigned-byte 12)))
                      form2
                      (if (and fix2 (typep fix2 '(unsigned-byte 12)))
                        form1))))
           (constant (and other (or fix1 fix2)))
           r1 r2)
      (cond ((null vreg)
             (arm642-form seg nil nil form1)
             (arm642-form seg nil xfer form2))
            ...
            (t
             (if constant
               ...
               (multiple-value-setq (r1 r2) (arm642-two-untargeted-reg-forms seg form1 arm64::arg_y form2 arm64::arg_z)))
             (ensuring-node-target (target vreg)
               (cond ((null overflow)
                      (if constant
...
(defun arm642-fixnum-sub (seg vreg xfer form1 form2 overflow)
  (with-arm64-local-vinsn-macros (seg vreg xfer)
    ...
    (let* ((fix1 (acode-fixnum-form-p form1))
           ...
           (other (unless diff
                    (if (and fix2
                             (typep fix2 '(unsigned-byte 12)))
                      form1)))
           (constant (and other fix2))
           r1 r2)
      (cond ((null vreg)
             (arm642-form seg nil nil form1)
             (arm642-form seg nil xfer form2))
          ...
          (let* ((cstack *arm642-cstack*)
            ...
            (if both-single-valued
              (let* ((result arm64::arg_z))
                ...
                (progn
                  (@ falselabel)
                  (arm642-predicate-block falselabel)
                ...
                (@ endlabel)
                (arm642-predicate-block endlabel)
                (<- result)
                (^))
              (progn
...
;;; ARM64: "short-float" → single-float (same idea, different naming)
  ...
  (if (null vreg)
    ...
    (multiple-value-bind (cr-bit true-p) (acode-condition-to-arm64-cr-bit cc)
      (let* ((fixbit (acode-fixnum-form-p bitnum)))
        (if fixbit
          (let* ((reg (arm642-one-untargeted-reg-form seg form arm64::arg_z))
                 ;; ARM64: 56-bit fixnum range (fixnumshift=0)
                 (arm-bit (min 55 (max fixbit 0))))
            (regspec-crf-gpr-case
             (vreg)
             (progn
...
;;; --- Natural (unsigned machine-word) operators ---

(defarm642 arm642-%natural+ %natural+ (seg vreg xfer x y)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil x)
      (arm642-form seg nil xfer y))
    (let* ((fix-x (acode-fixnum-form-p x))
      ...
      (if (and fix-x fix-y)
        (arm642-natural-constant seg vreg xfer (+ fix-x fix-y))
        (let* ((u12x (and (typep fix-x '(unsigned-byte 12)) fix-x))
               (u12y (and (typep fix-y '(unsigned-byte 12)) fix-y)))
          (if (not (or u12x u12y))
            (with-imm-target () (xreg :natural)
              ...
              (<- xreg))
            (let* ((other (if u12x y x)))
              (with-imm-target () (other-reg :natural)
                (arm642-one-targeted-reg-form seg other other-reg)
                (! %natural+-c other-reg other-reg (or u12x u12y))
                (<- other-reg))))
          (^))))))
...
(defarm642 arm642-%natural- %natural- (seg vreg xfer x y)
  (if (null vreg)
    ...
    (let* ((fix-x (acode-fixnum-form-p x))
      ...
      (if (and fix-x fix-y)
        (arm642-natural-constant seg vreg xfer (- fix-x fix-y))
        (let* ((u12y (and (typep fix-y '(unsigned-byte 12)) fix-y)))
          (if (not u12y)
            (with-imm-target () (xreg :natural)
              (with-imm-target (xreg) (yreg :natural)
                (arm642-two-targeted-reg-forms seg x xreg y yreg)
            ...
            (progn
              (with-imm-target () (xreg :natural)
                (arm642-one-targeted-reg-form seg x xreg)
                (! %natural--c xreg xreg u12y)
                (<- xreg))))
          (^))))))

(defarm642 arm642-%natural-logior %natural-logior (seg vreg xfer x y)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil x)
      (arm642-form seg nil xfer y))
    (let* ((naturalx (nx-natural-constant-p x))
      ...
      (if (and naturalx naturaly)
        (arm642-natural-constant seg vreg xfer (logior naturalx naturaly))
        ;; ARM64: ORR immediate encoding is complex; use register form
        (with-imm-target () (xreg :natural)
          (with-imm-target (xreg) (yreg :natural)
            (arm642-two-targeted-reg-forms seg x xreg y yreg)
...
(defarm642 arm642-%natural-logxor %natural-logxor (seg vreg xfer x y)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil x)
      (arm642-form seg nil xfer y))
    (let* ((naturalx (nx-natural-constant-p x))
           (naturaly (nx-natural-constant-p y)))
      (if (and naturalx naturaly)
        (arm642-natural-constant seg vreg xfer (logxor naturalx naturaly))
        (with-imm-target () (xreg :natural)
          (with-imm-target (xreg) (yreg :natural)
            (arm642-two-targeted-reg-forms seg x xreg y yreg)
...
(defarm642 arm642-%natural-logand %natural-logand (seg vreg xfer x y)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil x)
      (arm642-form seg nil xfer y))
    (let* ((naturalx (nx-natural-constant-p x))
           (naturaly (nx-natural-constant-p y)))
      (if (and naturalx naturaly)
        (arm642-natural-constant seg vreg xfer (logand naturalx naturaly))
        (with-imm-target () (xreg :natural)
          (with-imm-target (xreg) (yreg :natural)
            (arm642-two-targeted-reg-forms seg x xreg y yreg)
...
(defarm642 arm642-natural-shift-right natural-shift-right (seg vreg xfer num amt)
...
(defarm642 arm642-natural-shift-left natural-shift-left (seg vreg xfer num amt)
  (with-imm-target () (dest :natural)
    (arm642-one-targeted-reg-form seg num dest)
...
(defarm642 arm642-%set-schar %set-schar (seg vreg xfer str idx char)
  (multiple-value-bind (src unscaled-idx char)
      (arm642-three-untargeted-reg-forms seg str arm64::arg_x idx arm64::arg_y char arm64::arg_z)
    (case (arch::target-char-code-limit (backend-target-arch *target-backend*))
...
;;; --- Float binary ops (macro-defined) ---
...
(defarm642 arm642-%alloc-misc %make-uvector (seg vreg xfer element-count st &optional initval)
  (if (null vreg)
    (progn
    ...
    (let* ((subtag (acode-fixnum-form-p st))
           ...
           (nbytes (if (and subtag nelements) (arm642-misc-byte-count subtag nelements))))
      (if (and nbytes (null initval)
                (< (logand
                    (lognot (1- (* 2 *arm642-target-node-size*)))
                    (+ nbytes *arm642-target-node-size*
                       (1- (* 2 *arm642-target-node-size*)))) #x8000))
        ...
        (progn
          (if initval
            (progn
              (arm642-three-targeted-reg-forms seg element-count ($ arm64::arg_x) st ($ arm64::arg_y) initval ($ arm64::arg_z))
              (! misc-alloc-init)
              (<- ($ arm64::arg_z)))
            (progn
              (arm642-two-targeted-reg-forms seg element-count ($ arm64::arg_y) st ($ arm64::arg_z))
              (! misc-alloc)
              (<- ($ arm64::arg_z))))))
      (^))))
    ...
    (when vreg
      (ensuring-node-target (target vreg)
        (if *arm642-open-code-inline*
          ...
          (let* ((target-other (not (eql (hard-regspec-value target) arm64::arg_z)))
...
(defarm642 arm642-ash ash (seg vreg xfer num amt)
  (arm642-binary-builtin seg vreg xfer 'ash num amt))

(defarm642 arm642-fixnum-ash fixnum-ash (seg vreg xfer num amt)
  (arm642-binary-builtin seg vreg xfer 'ash num amt))

;;; ======================================================================
;;; Section 11: Advanced Operators (Chunk 11 of 12)
;;; ======================================================================
...
(defarm642 arm642-gvector-typecode-p gvector-typecode-p (seg vreg xfer val)
  (cond ((null vreg) (arm642-form seg vreg xfer val))
        (t (ensuring-node-target (target vreg)
             (! gvector-typecode-p target (arm642-one-untargeted-reg-form seg val arm64::arg_z)))
           (^))))

(defarm642 arm642-ivector-typecode-p ivector-typecode-p (seg vreg xfer val)
  (cond ((null vreg) (arm642-form seg vreg xfer val))
        (t (ensuring-node-target (target vreg)
             (! ivector-typecode-p target (arm642-one-untargeted-reg-form seg val arm64::arg_z)))
           (^))))

;;; --- Symbol operations ---

;;; On ARM64, symbols ARE symvectors (same underlying representation).
...
(defarm642 arm642-%symptr->symvector %symptr->symvector (seg vreg xfer arg)
  (arm642-identity seg vreg xfer arg))
...
(defarm642 arm642-%interrupt-poll %interrupt-poll (seg vreg xfer)
  ;; ARM64 doesn't have event-poll vinsn — just return nil.
  ;; Interrupt polling is handled by the kernel's signal mechanism.
  (arm642-form seg vreg xfer (make-acode (%nx1-operator nil))))
...
(defarm642 arm642-%single-to-double %single-to-double (seg vreg xfer arg)
  (if (null vreg)
    (arm642-form seg vreg xfer arg)
    (with-fp-target () (sreg :single-float)
      (setq sreg (arm642-one-untargeted-reg-form seg arg sreg))
      (if (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
               (= (get-regspec-mode vreg) hard-reg-class-fpr-mode-double))
        (<- sreg)
        (with-fp-target (sreg) (dreg :double-float)
          (arm642-copy-register seg dreg sreg)
          (<- dreg)))
      (^))))

(defarm642 arm642-%fixnum-to-double %fixnum-to-double (seg vreg xfer arg)
  ;; ARM64 fixnumshift=0, so the fixnum IS the raw integer.
  ;; fixnum->double is a single scvtf instruction.
  ...
  (with-fp-target () (sreg :single-float)
    (let* ((r (arm642-one-untargeted-reg-form seg arg arm64::arg_z)))
      (unless (or (acode-fixnum-form-p arg)
                  *arm642-reckless*)
        (! trap-unless-fixnum r))
      (! fixnum->single sreg r)
      (<- sreg)
      (^))))

;;; --- Float access operators ---
...
;;; On ARM64, single-float is immediate (tag 0x10), so %setf-short-float
;;; doesn't apply in the same way as on ARM32 where single-floats are boxed.
;;; For compatibility, we call the generic function.
(defarm642 arm642-%setf-short-float %setf-short-float (seg vreg xfer fnode fval)
  (arm642-call-fn seg vreg xfer
                  (make-acode (%nx1-operator immediate) '%setf-short-float)
                  (list nil (list fval fnode))
                  nil))

;;; short-float-compare on ARM64 — same as single-float-compare
...
;;; --- Fixnum-indexed float memory access ---

(defarm642 arm642-fixnum-ref-double-float %fixnum-ref-double-float (seg vreg xfer base index)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil base)
      (arm642-form seg nil xfer index))
    (let* ((fix (acode-fixnum-form-p index)))
      ...
      (if (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
               (= (get-regspec-mode vreg) hard-reg-class-fpr-mode-double))
        (cond (fix
               (! fixnum-ref-c-double-float vreg (arm642-one-untargeted-reg-form seg base arm64::arg_z) fix))
              (t
               (multiple-value-bind (rbase rindex) (arm642-two-untargeted-reg-forms seg base arm64::arg_y index arm64::arg_z)
                 (! fixnum-ref-double-float vreg rbase rindex))))
        (with-fp-target () (target :double-float)
          (cond (fix
                 (! fixnum-ref-c-double-float target (arm642-one-untargeted-reg-form seg base arm64::arg_z) fix))
                (t
                 (multiple-value-bind (rbase rindex) (arm642-two-untargeted-reg-forms seg base arm64::arg_y index arm64::arg_z)
...
(defarm642 arm642-fixnum-set-double-float %fixnum-set-double-float (seg vreg xfer base index val)
  (let* ((fix (acode-fixnum-form-p index)))
    (unless (typep fix '(integer 0 (128)))
      (setq fix nil))
    (cond ((or (null vreg)
               (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
                    (= (get-regspec-mode vreg) hard-reg-class-fpr-mode-double)))
           (let* ((fhint (or vreg ($ arm64::d0 :class :fpr :mode :double-float))))
             (if fix
               (multiple-value-bind (rbase rval)
                   (arm642-two-untargeted-reg-forms seg base ($ arm64::arg_z) val fhint)
                 (! fixnum-set-c-double-float rbase fix rval)
                 (<- rval))
               (multiple-value-bind (rbase rindex rval)
                   (arm642-three-untargeted-reg-forms seg base ($ arm64::arg_y) index ($ arm64::arg_z) val fhint)
                 (! fixnum-set-double-float rbase rindex rval)
                 (<- rval)))))
          (t
           (if fix
             (multiple-value-bind (rbase rboxed)
                 (arm642-two-untargeted-reg-forms seg base ($ arm64::arg_y) val ($ arm64::arg_z))
               (with-fp-target () (rval :double-float)
                 (arm642-copy-register seg rval rboxed)
                 (! fixnum-set-c-double-float rbase fix rval))
               (<- rboxed))
             (multiple-value-bind (rbase rindex rboxed)
                 (arm642-three-untargeted-reg-forms seg base ($ arm64::arg_x) index ($ arm64::arg_y) val ($ arm64::arg_z))
               (with-fp-target () (rval :double-float)
                 (arm642-copy-register seg rval rboxed)
                 (! fixnum-set-double-float rbase rindex rval))
               (<- rboxed)))))
...
(defarm642 arm642-%complex-double-float-realpart %complex-double-float-realpart (seg vreg xfer arg)
  (if (null vreg)
...
(defarm642 arm642-%complex-double-float-imagpart %complex-double-float-imagpart (seg vreg xfer arg)
  (if (null vreg)
    (arm642-form seg nil xfer arg)
    (with-fp-target () (target :double-float)
      (when (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
...
(defarm642 arm642-%complex-single-float-realpart %complex-single-float-realpart (seg vreg xfer arg)
  (if (null vreg)
    (arm642-form seg nil xfer arg)
...
(defarm642 arm642-%complex-single-float-imagpart %complex-single-float-imagpart (seg vreg xfer arg)
  (if (null vreg)
    (arm642-form seg nil xfer arg)
    (with-fp-target () (target :single-float)
      (when (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
...
(defarm642 arm642-%make-complex-double-float %make-complex-double-float (seg vreg xfer r i)
  (if (null vreg)
    (progn
...
(defarm642 arm642-%make-complex-single-float %make-complex-single-float (seg vreg xfer r i)
  (if (null vreg)
    (progn
      (arm642-form seg nil nil r)
      (arm642-form seg nil xfer i))
...
;;; complex, realpart, imagpart — call generic functions
...
(defarm642 arm642-catch catch (seg vreg xfer tag valform)
  (let* ((tag-label (backend-get-next-label))
    ...
    ;; ARM64 has no constant pool to lock/unlock
    (if mv-pass
      (! mkcatchmv)
      (! mkcatch1v))
    ...
    (arm642-lri seg arm64::imm0 1) ; fixnumshift=0, so (ash 1 0) = 1
    (if mv-pass
      (! nthrowvalues)
      (! nthrow1value))
    (arm642-close-undo)
  ...
  (when vreg
    (ensuring-node-target (target vreg)
      (with-node-temps () (symreg)
        ...
        (! node-slot-ref target symreg arm64::symbol.vcell-cell))))
  (^))
...
(defarm642 arm642-global-setq global-setq (seg vreg xfer sym val)
  (arm642-vset seg
               ...
               (make-acode (%nx1-operator fixnum)
                           arm64::symbol.vcell-cell)
               val
               nil))

;;; ======================================================================
;;; Chunk 12: FFI, pointers, multi-dim arrays, C frames, require-*, multiple-value-call
;;; ======================================================================
  ...
  (with-arm64-local-vinsn-macros (seg vreg xfer)
    (if (eql 0 (%ilogand #xf bits))
      ...
      (let* ((size (logand #xf bits))
        ...
        (flet ((val-to-argz-and-imm0 ()
                 (arm642-one-targeted-reg-form seg val ($ arm64::arg_z))
                 ;; On ARM64, fixnum->signed-natural is identity (fixnumshift=0),
                 ;; but we still need to handle larger sizes.
                 (if (eq size 8)
                   (if signed
                     (! gets64)
                     (! getu64))
...
;;; ---- Word / fixnum reference operators ----
...
(defarm642 arm642-%lisp-word-ref %lisp-word-ref (seg vreg xfer base offset)
  (let* ((fixoffset (acode-fixnum-form-p offset)))
    (cond ((null vreg)
           (arm642-form seg nil nil base)
           (arm642-form seg nil xfer offset))
          ;; ARM64: signed-byte 10 for scaled LDR offset
      ...
      (if bit-index
        (let* ((mask-start (logand 31 (+ bit-index 25)))
          ...
          (if constval
            ...
            (let* ((*arm642-nfp-depth* *arm642-nfp-depth*))
              (unless triv-val
                (arm642-push-register seg src))
              (let* ((target (arm642-one-untargeted-reg-form seg newval arm64::arg_z)))
                (unless triv-val
                  (arm642-pop-register seg src))
        ...
        (let* ((*arm642-nfp-depth* *arm642-nfp-depth*))
          ...
          (multiple-value-bind (idx-reg val-reg)
              (arm642-two-untargeted-reg-forms seg offset arm64::arg_y newval arm64::arg_z)
            (unless (and triv-val triv-offset)
              (arm642-pop-register seg src))
...
(defarm642 arm642-%reference-external-entry-point %reference-external-entry-point
  (seg vreg xfer arg)
  (ensuring-node-target (target vreg)
    (let* ((reg (if (eq (hard-regspec-value target) arm64::arg_z) ($ arm64::arg_y) ($ arm64::arg_z))))
...
;;; ---- C frame operators ----
;;; ARM64 on macOS uses AAPCS64 calling convention.

(defarm642 arm642-with-c-frame with-c-frame (seg vreg xfer body &aux
                                                 (old-stack (arm642-encode-stack)))
  (! alloc-aapcs64-c-frame 0)
...
(defarm642 arm642-with-variable-c-frame with-variable-c-frame (seg vreg xfer size body &aux
  ...
  (let* ((reg (arm642-one-untargeted-reg-form seg size arm64::arg_z)))
    (! alloc-variable-aapcs64-c-frame reg)
    (arm642-open-undo $undo-arm64-c-frame)
    (arm642-undo-body seg vreg xfer body old-stack)))

;;; ---- FF call ----
;;; ARM64 macOS uses AAPCS64 (not EABI).  The operator name is "eabi-ff-call"
;;; for historical reasons, but we generate AAPCS64 code.

(defarm642 arm642-eabi-ff-call eabi-ff-call (seg vreg xfer address argspecs argvals resultspec &optional monitor)
  ...
  (let* ((*arm642-vstack* *arm642-vstack*)
    ...
    (arm642-vpush-register seg (arm642-one-untargeted-reg-form seg address arm64::arg_z))
    ;; Count space needed
      ...
      (case spec
        ...
        (t
         (if (typep spec 'fixnum)
           ...
           (incf next-arg-word)))))
    (! alloc-aapcs64-c-frame (+ next-arg-word 8))
    (arm642-open-undo $undo-arm64-c-frame)
           ...
           (if (typep spec 'fixnum)
             (with-imm-target () (addr :address)
               ...
               (with-imm-target (addr) (valreg :natural)
                 (dotimes (i spec)
                   (! mem-ref-c-natural valreg addr (* i *arm642-target-node-size*))
                   (! set-aapcs64-c-arg valreg next-arg-word)
                   (incf next-arg-word))))
             (with-imm-target () (valreg :natural)
               (let* ((reg (arm642-unboxed-integer-arg-to-reg seg valform valreg spec)))
...
;;; ---- require-* type-check operators (macrolet) ----
