;;; Recompile l1-clos-boot.lisp for ARM64 (Bug 192: avoid unbound *random-state*)
;;; Usage: ./dx86cl64 -l recompile-l1-clos-boot.lisp
(in-package :ccl)
...
;;; Step 4: Compile l1-clos-boot.lisp targeting ARM64 using cross-compile mechanism
(format t "~%=== Compiling l1-clos-boot.lisp for ARM64 ===~%")
(with-cross-compilation-target (:darwinarm64)
  (let* ((*target-backend* (find-backend :darwinarm64)))
    (target-compile-modules '(l1-clos-boot) :darwinarm64 t)))
(format t "~%=== Done ===~%")
(quit)
